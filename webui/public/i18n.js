...
const I18N = {
  zh: {
    ...
    "network.hint": "（数据来自系统层探针 `CC-Monitor probe`，只有 Linux + eBPF 且探针在运行时才有数据；不是抓包/中间人，只看得到 Claude Code 进程树自己发起的连接）",
    "network.mapTitle": "连接目的地世界地图",
    "network.tableTitle": "按目标地址明细",
    "network.mapNoWebgl": "这个浏览器不支持 WebGL2，世界地图画不出来（下面的表格数据不受影响）",
    "network.mapEmpty": "还没有能定位到经纬度的连接（可能是没配置 GeoIP 数据库，或者探针还没跑）",
    ...
    "network.empty": "还没有观测到网络流量数据（探针没在跑，或者 Claude Code 还没发起过网络请求）",
  ...
  en: {
    ...
    "network.hint": "(data comes from the system-layer probe, `CC-Monitor probe` — Linux + eBPF only, and only while the probe is running; this is not packet capture / MITM, it only sees connections the Claude Code process tree itself makes)",
    ...
    "network.mapEmpty": "No connections could be located yet (no GeoIP database configured, or the probe hasn't run yet)",
    ...
    "network.empty": "No network traffic observed yet (the probe isn't running, or Claude Code hasn't made any network requests)",
