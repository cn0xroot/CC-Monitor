...
const I18N = {
  zh: {
    ...
    "drilldown.aiTrajectory.noSessionHint": "这部分数据来自系统层探针（内核层面的观测），探针只看得到 pid/uid，天生不知道对应哪个 Claude Code session，Session ID/文件夹路径这两栏用不了——上面按目标地址聚合的表格不受影响。",
  ...
  en: {
    ...
    "drilldown.aiTrajectory.noSessionHint": "This data comes from the system-layer probe (a kernel-level observation) — it only sees pid/uid, with no inherent way to know which Claude Code session a connection belongs to, so the Session ID / working-directory columns aren't available here. The target-address table above is unaffected.",
