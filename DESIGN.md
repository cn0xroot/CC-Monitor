...
`sched_process_fork` 把这层派生关系持续传播给后续所有子进程，实测能干净地把 Claude Code 自己
...
- 策略引擎目前对命令文本做的是纯正则 `re.search`，没有引号/heredoc 感知——多行字符串参数或
  heredoc 正文里的内容理论上可能被规则误判成命令本身的一部分（Web UI 那边处理命令分类展示时
  已经用一个简化版 shell 分词器 `splitShellSegments()` 解决了同类问题，但策略引擎这边还没有
  同步做，这是全部规则共有的既有特点，不是某一条新规则单独引入的问题）。
