...
还在为 AI 开发时不知道 AI Agent 在你的电脑上执行了哪些操作而烦恼吗？试试这个工具吧，实时监测
...
- **70+ 条内置检测规则，三级风险自动分流**：高危直接拦截（`rm -rf`、反弹 shell、写 SSH 密钥……），中危弹窗要求人工确认，低危静默记录，不用每条操作都手动盯着。
...
装完之后，你随时可以决定这个工具管得有多严。一共三档，任何时候只有一档生效。

打个比方，它像门口的保安：

| 档位 | 保安在干什么 | 颜色 |
|---|---|---|
| **拦截中** | 看见可疑的人直接拦下，拿不准的先问你一声，其余的记在本子上 | 紫 |
| **观察模式** | 谁进谁出全都记在本子上，但一个都不拦，也不来问你 | 绿 |
| **已关闭** | 保安下班了，本子也收起来了 | 黄 |

说得具体一点：

- **拦截中**是默认档，装完不动任何设置就是这一档。命中高危规则的操作会被直接拦下来，
  比如 `rm -rf /`、反弹 shell、往 `~/.ssh` 里写东西。命中中危规则的会弹个确认框问你，
  你说行才放行。剩下的都静默记录，不打扰你。
- **观察模式**只记不拦。规则判定照常跑，风险等级、命中了哪条规则，一条不落地记进
  审计日志，只是**从不真的拦截，也不弹任何确认框**。适合两种情况：一是你正在赶进度，
  不想被确认框打断，但事后想知道 AI 都干了什么；二是你刚装上，还不清楚自己的日常
  工作流会撞上哪些规则，先挂两天看看记录，再决定要不要收紧。
- **已关闭**是唯一真正停掉审计的一档。不判定、不记录，跟没装这个工具完全一样。
  它的用途很窄：怀疑是 CC-Monitor 自己导致了某个问题，关掉对比一下。

**最容易搞混的是「观察模式」和「已关闭」。** 两者都不会拦你，区别在于事后有没有东西可看。
观察模式下日志照记，关掉之后那段时间是一片空白，你没法回头查 AI 做过什么。
所以想少被打扰，选观察模式，不要选关闭。

切换方式有两种。命令行：

```bash
CC-Monitor audit start        # 拦截中
CC-Monitor audit permissive   # 观察模式（旧名字 pause 仍然可用）
CC-Monitor audit stop         # 已关闭
CC-Monitor audit status       # 看当前是哪一档
```

网页上在首页工具栏左侧，三档并排，点一下就切，当前档会高亮成对应的颜色。切到
「已关闭」会多问一次，因为那一档会让审计断档。顶栏还常驻一个同色的状态指示灯，
不用回首页也能一眼看到现在是哪一档。

有一点要注意：**系统层探针不受这个开关控制**。只要探针在跑，无论你切到哪一档，
它都会继续在内核层记录命令执行和网络连接。这是故意的——探针的价值就在于不依赖
hook 的配合做独立观察，跟着一起关掉就没意义了。
...
- **首页**：
  - **介入级别**：拦截中 / 观察模式 / 已关闭三档，做成并排的分段控件，三档同时可见、
    点哪档去哪档，当前档高亮成对应颜色（紫 / 绿 / 黄）。控件下方一行说明会跟着当前档
    变化。切到「已关闭」要二次确认。顶栏常驻一个同色的状态指示灯。三档的含义见上面
    [介入级别](#介入级别三档怎么选)一节。
  ...
  - **数据管理**：把当前事件数据持久化归档（SQLite `backup()` API 做完整快照）或者
    清空重新统计；日志类型与风险等级分布图。
- **Log 审计**：全宽的实时审计日志查看（按会话过滤，会话下拉框显示"文件夹 · 模型 · 短ID"而不是一串看不出区别的 ID），复用 CLI 那套人类可读的事件翻译逻辑。数据来自 `events.db`（CC-Monitor 的 hooks 抓到、经过策略引擎判定后写入），内容按隐私原则**故意脱敏**——Write/Edit 只显示"路径 (N 字节)"、TodoWrite 只显示任务条数、截屏只显示元数据，从不展示实际读写内容。回答的是"发生了什么操作、风险等级/放行拦截结果"这个安全审计问题；想看某个会话完整聊了什么，去下面的 **Claude Tap**。
...
- **作者不对因使用/误用本工具造成的任何直接或间接损失负责**（规则误拦截导致
  ...
  在拦什么。
- **执行本项目部分功能（如读取账号额度信息）可能导致原有 Claude Code 登录
  状态失效，需要重新登录**。这些功能读的是 Claude Code 自己保存的本地凭证
  （Linux 上是 `~/.claude/.credentials.json`，macOS 上是登录钥匙串），本身
  不修改凭证文件，但不排除触发方读取/并发访问等边界情况下出现登录状态异常
  的可能。如果账号登录状态比较重要（比如正在长时间跑的任务），建议先确认
  能接受重新登录一次的风险，再决定要不要开启这部分功能。
